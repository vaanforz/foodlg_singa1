...
def __create_res_next(nb_classes, img_input, include_top, depth=29, cardinality=8, width=4,
    ...
    if type(depth) is list or type(depth) is tuple:
        ...
        # Otherwise, default to 3 blocks each of default number of group
        # convolution blocks
    ...
    # block 1 (no pooling)
    for i in range(N[0]):
        x = __bottleneck_block(
            x, filters_list[0], cardinality, strides=1, weight_decay=weight_decay)

    N = N[1:]  # remove the first block from block definition list
    # remove the first filter from the filter list
