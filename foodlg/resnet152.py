...
class Scale(Layer):
    ...
    def build(self, input_shape):
        ...
        self.gamma = K.variable(self.gamma_init(
            shape), name='%s_gamma' % self.name)
        self.beta = K.variable(self.beta_init(
            shape), name='%s_beta' % self.name)
...
def ResNet152(include_top=True, weights=None,
    ...
    if weights and include_top and classes != 1000:
...
if __name__ == '__main__':
    ...
    if K.image_dim_ordering() == 'th':
        # Transpose image dimensions (Theano uses the channels as the 1st
        # dimension)
